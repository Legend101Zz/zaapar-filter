...
class SnapshotManager extends elementFactory_1.default {
    ...
    captureimage(captureParameters, localisation) {
        ...
        SnapshotManager._dataURItoFileBlob(captureParameters.data, '.jpeg', captureParameters.fileNamePrepend).then((file) => {
        ...
        if (!profile.iOS12HideSave) {
            ...
            if (profile.iOSFilesAppForPhotoShare) {
                ...
                buttonSave.addEventListener('click', () => {
                    ...
                    if (profile.ios) {
                        buttonSave.remove();
                        this.containerButtons.prepend(SnapshotManager._getOpenFilesButton());
                    }
                });
            }
        }
    ...
    shareVideo(captureParameters, localisation) {
        ...
        this.containerButtons.appendChild(this.instructionsText);
        SnapshotManager._dataURItoFileBlob(captureParameters.data, '.mp4', captureParameters.fileNamePrepend).then((file) => {
            if (captureParameters.hideShareButton)
                return;
    ...
    static _getFileName(fileType, prepend = 'Zappar') {
        return `${prepend}.${fileType}`;
    }
    static async _dataURItoFileBlob(dataURI, fileType, fileName) {
        const blob = await fetch(dataURI).then((res) => res.blob());
        const file = new File([blob], `${SnapshotManager._getFileName(fileType, fileName)}`, { type: blob.type });
        return file;
    }
}
